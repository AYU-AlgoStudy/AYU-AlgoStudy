...
### 문제 풀이
> [백준/Silver/15654. N과 M （5）/N과 M （5）.cc](https://github.com/Okchun-Yee/CodingTestStudy/blob/eec0d2b9ef134759cb8c7c549dfd7a418979a255/%EB%B0%B1%EC%A4%80/Silver/15654.%E2%80%85N%EA%B3%BC%E2%80%85M%E2%80%85%EF%BC%885%EF%BC%89/N%EA%B3%BC%E2%80%85M%E2%80%85%EF%BC%885%EF%BC%89.cc)

### 개념 정리
> **백트리킹 기본**
    
    사전순서로 결과 출력이기에 입력 후 재귀탐색 전에 미리 정렬을 해두고 탐색을 진행하는 간단한 문제.
    다만 첫 오류는 상태복원 과정에서 이전 과정에서 추가한 정답을 제거하지 않아서 계속 중첩되는 문제.
    두 번째는 백준에서 **시간초과** 발생으로 원인은 출력과정에서 사용한 endl 이 문제
``` C++
    // D0 - 정답 배열에 추가
...
> 문제 풀이 과정에서 개선점
    ...
    출력할때 최대한 줄바꿈은 endl 대신 '\n' 사용하기. > 시간 초과 방지용

### 총평
> 시험 기간이라 간단한 문제 위주로 푸는 중
...
### 문제 풀이
> [백준/Silver/2503. 숫자 야구/숫자 야구.cc](https://github.com/Okchun-Yee/CodingTestStudy/blob/14294f65def0ab71da2522eb4de011d2199aedff/%EB%B0%B1%EC%A4%80/Silver/2503.%E2%80%85%EC%88%AB%EC%9E%90%E2%80%85%EC%95%BC%EA%B5%AC/%EC%88%AB%EC%9E%90%E2%80%85%EC%95%BC%EA%B5%AC.cc)

### 개념 정리
> **브루트 포스 전체 탐색 개념**
    
    용의자 - 단서 - 생존자 방식의 알고리즘
    용의자 (가능한 전체 정답)를 하나씩 탐색하면서 단서 (주어진 입력)와 비교하며 모든 단서와 일치하는 해당 용의자는 생존자로 판단하는 알고리즘.

    문제는 세자리 숫자가 중복 될 수 없으므로 9*8*7 = 504개의 용의자가 존재 이를 입력으로 주어진 단서와 하나씩 대조 하며 모든 단서와 일치했던 생존자 찾기.
    ...
    이때 스트라이크, 볼이 존재하기에 모든 용의자를 위 4개와 비교해 스트라이크, 볼이 똑같이 나오는 숫자 찾기
``` C++
    int cnt_s = 0, cnt_b = 0;
			for (int r = 0; r < 3; ++r) {
				for (int c = 0; c < 3; ++c) {
...
> 문제 풀이 과정에서 개선점
    ...
    처음에는 한자리씩 경우의 수를 따지려 했지만 브루트 포스 문제 같은 경우에는 전체의 범위가 적을 경우 가장 간단한 전체 탐색 방법부터 생각해 보는것이 훨씬 효율적인 방법이라는 것을 알게 되었습니다.

### 총평    
> 스트라이크와 볼을 비교하는 개념을 이해하며 용의자라는 개념을 이해하는데 시간이 걸렸던 문제 였습니다.
...
- 이유 : 숫자 야구에서 볼/스트라이크 판별에 대한 개념을 잡는데 시간이 소요. 용의자가 4개의 단서를 전부 통과해야하는건데 각 단서에 대한 용의자로 생각을 하여서 핀트를 벗어났었음.
