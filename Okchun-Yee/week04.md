...
``` C++
    ...
    for(int i=0;i<=SIZE - index;++i)
        다음 index = index + 1 (이 부분은 가로, 세로, 대각선에 따라 다름)
        범위 검사
        다음 돌이 현재 돌과 동일 한지 검사
```
...
### 주차 review
> 제일 어려웠던 문제 : BOJ 11000 "강의실 배정"
- 이유: 접근 방식을 알아 내는 과정이 조금 어려웠다. 첫 시작을 인접 리스트에 매몰되어서 접근한 이유가 큰것같다. 우선순위 큐를 사용해야 한다는 것을 빨리 알았으면 더 빨리 해결할 수 있었겠지만, 인접 리스트 방식을 사용하려 해서 오답이 반복되었던 것 같다.

> 새로이 알게된 알고리즘 : 다익스트라, 플루이드 와샬, SPFA, 그리디 탐색
