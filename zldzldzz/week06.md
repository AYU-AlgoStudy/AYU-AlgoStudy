...
## 6주차 5번 문제 BOJ 18243번

🔗| 문제 Link
[문제 링크](https://www.acmicpc.net/problem/18243)

### 문제 풀이
https://github.com/zldzldzz/Algorithm/commit/e70318c3ca15dd6469fa903e10717851aa8308b4

### 개념 정리

    3중 반복문을 통해 문제를 금방 코드화 하였다. 하지만 경유지가 가장 밖의 인덱스로 해야 하는 이유를 알지 못해서 틀렸다.
    이유는 이미 계산된 최단 경로(가장 밖의 인덱스)를 통해서 새로운 경로를 해야 하기 때문이다. 만약 가장 밖이 아니라면 아직 계산 되지 않은
    다른 값을 조회 되기 때문에 계산하지 못한 부분들이 생긴다.
    또한 가장 큰 값을 Integer.MAX_VALUE로 초기화 했지만 오버플로우가 발생해서 내가 예상하지 못한 결과나 나와 Integer.MAX_VALUE/2로 변경했다.

### 총평

생각보다 쉽게 접근했지만 가장 밖의 인덱스를 경유지로 사용해여 한다는 것을 처음 알게 되었다.

---

### 이번주 총평

매주 dp문제만 풀다가 다른 문제들을 풀다 보니 이전의 감각을 잊어 쉽지 않은 것 같다.